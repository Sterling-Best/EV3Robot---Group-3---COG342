...
class Robot:
    ...
    def moveforward(self, distance: int) -> None:
        ...
        currentdistance = round(self.ultrasonic.value()/10)
    ...
    def tankrotate(self, degrees: int) -> None:
        rotation = degrees * 2
        self.motorLeft.run_to_rel_pos(speed_sp=100, position_sp=rotation)
        self.motorRight.run_to_rel_pos(speed_sp=100, position_sp=-rotation)
        self.motorLeft.wait_while(self.motorLeft.STATE_RUNNING)
        self.motorRight.wait_while(self.motorRight.STATE_RUNNING)
